// ================================================================
// FILE: songgajah-core.js (Simpan di GitHub/Hosting External)
// ================================================================
...
window.processProfileImage = function(url, userName) {
...
window.renderGuestHeader = function(data) {
    ...
    if (logoUrl && imgEl) {
        const newSrc = window.processProfileImage(logoUrl, 'Logo');
        if (imgEl.src !== newSrc) {
            imgEl.src = newSrc;
            imgEl.onload = () => {
        ...
        const observer = new IntersectionObserver((entries, obs) => {
            entries.forEach(async entry => {
                if (entry.isIntersecting) {
                    ...
                    try {
                        await window.loadLeafletLibrary();
...
window.loadLandingNews = async function() {
    ...
    try {
        ...
        entries.forEach(entry => {
            ...
            let snippet = '';
            if (entry.summary) { snippet = entry.summary.$t.substring(0, 100); } 
            else if (entry.content) { const tempDiv = document.createElement('div'); tempDiv.innerHTML = entry.content.$t; snippet = tempDiv.textContent.substring(0, 100); }
            html += `<article class="flex flex-col bg-white dark:bg-slate-800 rounded-2xl overflow-hidden hover:shadow-md hover:-translate-y-1 transition-all duration-300 border border-slate-100 dark:border-slate-700/50"><a href="${link}" aria-label="Baca artikel: ${safeTitle}" class="relative h-48 overflow-hidden group"><img src="${img}" alt="Thumbnail ${safeTitle}" loading="lazy" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"><div class="absolute top-3 left-3 bg-blue-600 text-white text-[10px] font-bold px-2 py-1 rounded-md shadow-sm">TERBARU</div></a><div class="flex flex-col flex-1 p-5"><div class="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-400 mb-3"><i class="far fa-calendar-alt" aria-hidden="true"></i> ${date}</div><h3 class="text-lg font-bold text-slate-900 dark:text-white mb-2 line-clamp-2 leading-snug hover:text-blue-600 transition"><a href="${link}" aria-label="Baca selengkapnya tentang ${safeTitle}">${title}</a></h3><p class="text-sm text-slate-700 dark:text-slate-300 line-clamp-3 mb-4 flex-grow">${snippet}...</p><a href="${link}" aria-label="Baca selengkapnya: ${safeTitle}" class="inline-flex items-center text-sm font-bold text-blue-700 hover:text-blue-800 mt-auto">Baca Selengkapnya <i class="fas fa-arrow-right ml-2 text-xs transition-transform group-hover:translate-x-1" aria-hidden="true"></i></a></div></article>`;
        });
        container.innerHTML = html;
        if (typeof window.fixBloggerAccessibility === 'function') window.fixBloggerAccessibility();
...
window.fetchVillageProfilePublic = async function() {
    ...
    if (cached) { try { window.renderGuestHeader(JSON.parse(cached)) } catch (e) {} }
    try {
        const res = await apiCall({ action: "get_identitas_desa" });
        if (res.status || res.success) { const finalData = res.data || res; window.renderGuestHeader(finalData); localStorage.setItem('CACHE_ID_DESA', JSON.stringify(finalData)) }
...
window.initRelatedPosts = function() {
    ...
    if (!labelRaw.trim()) { wrapper.classList.add("hidden"); return; }
    const labels = labelRaw.split(",").map(s => s.trim()).filter(s => s).slice(0, 3);
    const currentUrl = window.location.href.split("?")[0];
    const promises = labels.map(label => fetch(`/feeds/posts/default/-/${encodeURIComponent(label)}?alt=json&max-results=5`).then(res => res.json()).then(data => data.feed.entry || []).catch(err => []));
    Promise.all(promises).then(results => {
        ...
        flatList.forEach(post => {
            ...
            if (href !== currentUrl && !seenTitles.has(title)) { seenTitles.add(title); uniquePosts.push(post); }
        });
        if (uniquePosts.length === 0) { wrapper.classList.add("hidden"); return; }
        ...
        container.innerHTML = finalPosts.map(post => {
            ...
            if (post.media$thumbnail) { imgUrl = post.media$thumbnail.url.replace(/\/s[0-9]+(-c)?\//, "/w600-h340-p-k-no-nu/"); } 
            else if (post.content && /src="([^"]+)"/.test(post.content.$t)) { imgUrl = post.content.$t.match(/src="([^"]+)"/)[1]; }
            return `<div class="group flex flex-col h-full bg-white dark:bg-gray-800 rounded-lg overflow-hidden border border-transparent hover:border-gray-200 dark:hover:border-gray-600 hover:shadow-xl transition-all duration-300"><a href="${link}" class="block w-full relative overflow-hidden bg-gray-100" style="padding-top: 56.25%;"><img src="${imgUrl}" alt="${title.replace(/"/g, '')}" loading="lazy" class="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"></a><div class="p-5 flex flex-col flex-grow"><h4 class="text-base font-bold text-blue-600 dark:text-blue-400 leading-snug mb-3 line-clamp-2 text-left"><a href="${link}">${title}</a></h4><div class="mt-auto pt-4 border-t border-gray-100 dark:border-gray-700 flex items-center gap-3"><div class="flex flex-col justify-center leading-none"><span class="text-xs text-blue-500 dark:text-blue-400 mb-1">${author}</span><span class="text-[10px] font-medium text-gray-500 dark:text-gray-400">${dateStr}</span></div></div></div></div>`;
        }).join("");
        wrapper.classList.remove("hidden");
    }).catch(err => { container.innerHTML = `<div class="col-span-full text-center py-6 text-sm text-gray-400 italic">Gagal memuat artikel terkait.</div>`; });
};
