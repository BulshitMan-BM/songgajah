...
function forceLogout() {
    // Panggil API logout agar Worker menghapus Cookie HttpOnly
    if (typeof window.apiCall === 'function') {
        window.apiCall({ action: "logout" }).finally(() => {
            doLocalCleanup();
        });
    } else {
        doLocalCleanup();
...
function doLocalCleanup() {
    const e = localStorage.getItem("darkMode"),
          t = localStorage.getItem("CACHE_ID_DESA");
    localStorage.clear();
    sessionStorage.clear();
    if(e) localStorage.setItem("darkMode", e);
    if(t) localStorage.setItem("CACHE_ID_DESA", t);
...
window.updateSidebarActiveState = function() {
    ...
    const currentTab = currentUrl.searchParams.get('tab'); // Ambil nilai ?tab=...

    // 2. Reset Semua State Menu (Matikan semua highlight)
    // Reset Menu Utama
    document.querySelectorAll('.nav-item').forEach(el => {
        el.classList.remove('active', 'text-primary', 'font-bold', 'bg-blue-50', 'dark:bg-gray-800', 'text-blue-600', 'dark:text-white');
        el.classList.add('text-gray-700', 'dark:text-gray-300'); 
    });
    // Reset Submenu
    document.querySelectorAll('.submenu a').forEach(el => {
        el.classList.remove('active', 'text-primary', 'font-bold', 'text-blue-600', 'dark:text-blue-400');
        el.classList.add('text-gray-600', 'dark:text-gray-400');
    });
    // Tutup Accordion & Reset Panah
    document.querySelectorAll('.submenu').forEach(s => s.classList.add('hidden'));
    document.querySelectorAll('.menu-toggle .fa-angle-left').forEach(icon => icon.style.transform = 'rotate(0deg)');

    // 3. Loop Semua Link di Sidebar
    ...
    allLinks.forEach(link => {
        ...
        try {
            ...
            const linkTab = linkUrl.searchParams.get('tab'); // Cek apakah link menu punya ?tab=...

            // Cek Kesamaan Path (File HTML yang sama)
            if (linkPath === currentPath) {
                
                let isMatch = false;

                // === LOGIKA KHUSUS HALAMAN LAPORAN/SURAT ===
                if (linkPath.includes('daftar-laporan-pengaduan.html')) {
                    
                    // Skenario 1: Browser sedang buka Tab SURAT (?tab=surat)
                    if (currentTab === 'surat') {
                        // Maka HANYA aktifkan menu yang punya link ?tab=surat
                        if (linkTab === 'surat') {
                            isMatch = true;
                        }
                    } 
                    // Skenario 2: Browser sedang buka Tab LAPORAN atau DEFAULT (kosong)
                    else {
                        // Maka HANYA aktifkan menu yang TIDAK punya ?tab=surat
                        // (yaitu menu "Kelola Laporan" yang linknya bersih)
                        if (linkTab !== 'surat') {
                            isMatch = true;
                        }
                    }
                    ...
                    // === LOGIKA HALAMAN LAIN (Exact Path Match) ===
                    isMatch = true;
                }

                // 4. JIKA COCOK, AKTIFKAN VISUAL
                if (isMatch) {
                    // A. Jika Menu Utama (Top Level)
                    if (link.classList.contains('nav-item')) {
                        link.classList.add('active', 'text-primary', 'font-bold', 'bg-blue-50', 'dark:bg-gray-800', 'text-blue-600', 'dark:text-white');
                        link.classList.remove('text-gray-700', 'dark:text-gray-300');
                    }
                    
                    // B. Jika Submenu (Child)
                    const parentSubmenu = link.closest('.submenu');
                    if (parentSubmenu) {
                        // Highlight Link
                        link.classList.add('active', 'text-primary', 'font-bold', 'text-blue-600', 'dark:text-blue-400');
                        ...
                        if (parentToggle) {
                            parentToggle.classList.add('text-primary', 'font-bold', 'dark:text-white');
                            parentToggle.classList.remove('text-gray-700', 'dark:text-gray-300');
                            
                            const arrow = parentToggle.querySelector('.fa-angle-left');
            ...
            console.error("Sidebar URL Error:", e);
        }
    });
};
function initUIEvents() {
    ...
    // ==========================================
    // 2. PERBAIKAN SINKRONISASI DARK MODE (FIX)
    // ==========================================
    
    // Fungsi untuk memaksa Ikon Dashboard sesuai dengan status HTML saat ini
    const syncDashboardTheme = () => {
        ...
        if (icon) {
            ...
            // Logika Ikon: 
            // Jika Gelap (isDark=true) -> Tampilkan Matahari (biar bisa klik jadi terang)
            // Jika Terang (isDark=false) -> Tampilkan Bulan
            if (isDark) {
    ...
    // [PENTING] Jalankan sinkronisasi ini SEGERA saat fungsi dipanggil
    syncDashboardTheme();

    // Event Listener Tombol Toggle Dashboard
    bindOnce('darkModeToggle', 'click', () => {
        ...
        localStorage.setItem('darkMode', currentStatus);
        
        // Update ikon Dashboard
        syncDashboardTheme();
        
        // [Tambahan] Coba update ikon Guest/Mobile juga jika ada (biar sync real-time)
        const guestIcon = document.querySelector('#guestDarkModeToggle i');
        const mobileIcon = document.querySelector('#mobileDarkModeToggle i');
        if(guestIcon) {
            guestIcon.classList.remove('fa-moon', 'fa-sun');
            guestIcon.classList.add(currentStatus ? 'fa-sun' : 'fa-moon');
        }
        if(mobileIcon) {
            mobileIcon.classList.remove('fa-moon', 'fa-sun');
            mobileIcon.classList.add(currentStatus ? 'fa-sun' : 'fa-moon');
        }
    });
    // ==========================================

    // 3. Sidebar Mobile Toggle (Menggunakan class 'open' untuk CSS baru)
    const toggleSidebar = () => { 
...
document.addEventListener("click", e => {
        ...
        const dm = document.getElementById("dropdownTambahMenu");
        if (dc && dm && !dm.classList.contains("hidden") && !dc.contains(e.target)) {
            dm.classList.add("hidden");
        }

        // 2. Dropdown Tambah Anggota Keluarga (Detail)
        const m = document.getElementById("menuTambahAnggota");
        const b = document.getElementById("btnMenuTambah");
        if (m && !m.classList.contains("hidden") && !m.contains(e.target) && (!b || !b.contains(e.target))) {
            m.classList.add("hidden");
        }

        // 3. Dropdown Profil (Kanan Atas)
        const pd = document.getElementById("profileDropdown");
        const pt = document.getElementById("profileToggle");
        if (pd && pt && !pt.contains(e.target) && !pd.contains(e.target)) {
            pd.classList.remove("show");
        }

        // 4. Dropdown Aksi Tabel (Pilih Aksi)
        ...
        // --- TAMBAHAN BARU: Dropdown Tambah KK ---
        const ddKK = document.getElementById('dropdownTambahKK');
        if (ddKK && !ddKK.classList.contains('hidden') && !ddKK.contains(e.target)) {
            ddKK.classList.add('hidden');
        }
        // -----------------------------------------
    });
...
window.initPenduduk = function() { 
        ...
        // 2. Binding Filter (Status, Sex, Wilayah)
        ...
        // ============================================================
        // 3. [PERBAIKAN] BINDING PAGINATION & LIMIT (YANG HILANG)
        // ============================================================
        
        // A. Dropdown Limit (Jumlah Tampilan)
        const limitSelect = document.getElementById('limitPenduduk');
        if (limitSelect && !limitSelect.dataset.bound) {
            limitSelect.addEventListener('change', (e) => {
                STATE.PENDUDUK.limit = parseInt(e.target.value);
                STATE.PENDUDUK.page = 1; // Reset ke halaman 1 jika limit berubah
                loadPenduduk();
            });
            limitSelect.dataset.bound = "true";
        }

        // B. Tombol Sebelumnya (Prev)
        const btnPrev = document.getElementById('btnPrevPenduduk');
        if (btnPrev && !btnPrev.dataset.bound) {
            btnPrev.addEventListener('click', () => {
                if (STATE.PENDUDUK.page > 1) {
                    STATE.PENDUDUK.page--;
                    loadPenduduk();
                }
            });
        ...
        // C. Tombol Selanjutnya (Next)
        const btnNext = document.getElementById('btnNextPenduduk');
        if (btnNext && !btnNext.dataset.bound) {
            btnNext.addEventListener('click', () => {
                // Cek apakah masih ada halaman selanjutnya berdasarkan info dari server
                const info = STATE.PENDUDUK.paginationInfo;
                if (info && STATE.PENDUDUK.page < info.total_page) {
                    STATE.PENDUDUK.page++;
                    loadPenduduk();
                }
            });
...
function renderPendudukTable(list) {
        ...
        // 1. Reset Header Checkbox setiap kali render ulang
...
<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-gray-700 transition-colors">
    ...
    <td class="px-4 py-3 text-xs text-gray-500">
        ...
        // 2. Ambil semua checkbox baris yang BARU SAJA dibuat
        const checkboxes = document.querySelectorAll('.row-checkbox');
        
        // 3. Fungsi Update Tombol Hapus (Lokal)
        ...
        // --- PERBAIKAN LOGIKA CHECK ALL DI SINI ---
        if(checkAll) {
            // Gunakan .onclick (bukan addEventListener) agar menimpa handler lama
            // Ini memastikan closure 'checkboxes' selalu merujuk pada data halaman saat ini
        ...
        // Pasang listener untuk setiap baris
 ...
 window.initKeluarga=function(){
  const c=document.getElementById("keluargaContainer");
  if(!c)return;
  c.classList.remove("hidden");

  const s=document.getElementById("searchKKInput");
  if(s&&!s.dataset.bound){
    s.addEventListener("input",debounce(e=>{
      STATE.KELUARGA.keyword=e.target.value;
      STATE.KELUARGA.page=1;
      loadKeluarga();
    },500));
  ...
  ["filter_kk_sex","filter_kk_status","filter_kk_dusun"].forEach(id=>{
    ...
    if(el&&!el.dataset.bound){
      el.addEventListener("change",e=>{
        STATE.KELUARGA.filters[id.replace("filter_kk_","")]=e.target.value;
        STATE.KELUARGA.page=1;
        loadKeluarga();
      });
      el.dataset.bound="true";
  ...
  const l=document.getElementById("limitKK");
  if(l&&!l.dataset.bound){
    l.addEventListener("change",e=>{
      STATE.KELUARGA.limit=parseInt(e.target.value);
      STATE.KELUARGA.page=1;
      loadKeluarga();
    });
  ...
  const p=document.getElementById("btnPrevKK");
  if(p&&!p.dataset.bound){
    p.addEventListener("click",()=>{
      if(STATE.KELUARGA.page>1){
        STATE.KELUARGA.page--;
        loadKeluarga();
      }
    });
  ...
  const n=document.getElementById("btnNextKK");
  if(n&&!n.dataset.bound){
    n.addEventListener("click",()=>{
      const i=STATE.KELUARGA.paginationInfo;
      if(i&&STATE.KELUARGA.page<i.total_page){
        STATE.KELUARGA.page++;
        loadKeluarga();
      }
    });
  ...
  const isiDusun=()=>{
    if(STATE.CACHE_WILAYAH&&STATE.CACHE_WILAYAH.dusun){
      isiDropdown("filter_kk_dusun",Object.keys(STATE.CACHE_WILAYAH),"Semua Dusun");
    }
  };

  if(!STATE.CACHE_WILAYAH) loadWilayahData().then(isiDusun);
  else isiDusun();

  if(!STATE.KELUARGA.data.length||STATE.KELUARGA.needsRefresh) loadKeluarga();
  else{
    renderKeluargaTable(STATE.KELUARGA.data);
    STATE.KELUARGA.paginationInfo&&updatePaginationUIKK(STATE.KELUARGA.paginationInfo);
  }
};
...
function renderKeluargaTable(list) {
        ...
        // 1. Reset status tombol hapus & checkbox header setiap render ulang
        if(checkAll) checkAll.checked = false;
        ...
        const rows = list.map(row => {
            ...
            const safeNik = String(row.nik || "").replace(/['"]/g, "");
            const safeNoKK = String(row.no_kk || "").replace(/['"]/g, "");
            const safeNama = String(row.nama || "").replace(/['"]/g, "`");
...
<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-blue-50 dark:hover:bg-gray-700 transition">
    ...
    <td class="px-4 py-3 text-center border-r dark:border-gray-600 whitespace-nowrap">
        <button onclick="lihatAnggotaKeluarga('${safeNoKK}', '${safeNama}')" class="bg-purple-600 hover:bg-purple-700 text-white px-2 py-1 rounded text-xs" title="Lihat Anggota"><i class="fas fa-list"></i></button>
        
        <button onclick="editKepalaKeluarga('${safeNik}')" class="bg-orange-500 hover:bg-orange-600 text-white px-2 py-1 rounded text-xs" title="Edit Kepala Keluarga"><i class="fas fa-edit"></i></button>
        
        <button onclick="lokasiPenduduk('${safeNik}')" class="bg-green-600 hover:bg-green-700 text-white px-2 py-1 rounded text-xs" title="Lokasi"><i class="fas fa-map-marker-alt"></i></button>
    </td>
    <td class="px-4 py-3 text-center border-r dark:border-gray-600">
        <img src="${fotoUrl}" class="w-8 h-8 rounded-full mx-auto shadow-sm">
    </td>
    <td class="px-4 py-3 font-mono text-blue-600 font-bold border-r dark:border-gray-600 cursor-pointer hover:text-blue-800 hover:underline">
        <a href="javascript:void(0)" onclick="lihatAnggotaKeluarga('${safeNoKK}', '${safeNama}')">${safeNoKK}</a>
    </td>
    <td class="px-4 py-3 font-bold border-r dark:border-gray-600 uppercase">${escapeHtml(row.nama)}</td>
    <td class="px-4 py-3 font-mono border-r dark:border-gray-600">${safeNik}</td>
    <td class="px-4 py-3 text-center font-bold border-r dark:border-gray-600 bg-blue-50 dark:bg-gray-700">
          <span class="px-2 py-1 bg-blue-200 text-blue-800 rounded-full text-xs" id="count-${safeNoKK}"><i class="fas fa-spinner fa-spin"></i></span>
    </td>
    ...
    <td class="px-4 py-3 text-xs text-gray-500">${tglDaftar}</td>
        ...
        // Ambil checkbox baris yang BARU dirender
        ...
        if(checkAll) {
            // Gunakan onclick agar menimpa event handler lama dan menggunakan 'checkboxes' yang baru
            checkAll.onclick = function() {
                checkboxes.forEach(cb => cb.checked = this.checked);
...
window.tambahAnggota=tipe=>{
  STATE.ORIGIN_VIEW="keluarga_detail";
  const m=document.getElementById("menuTambahAnggota");
  m&&m.classList.add("hidden");

  if(tipe==="lahir"){
    if(!STATE.ACTIVE_KK_DATA||!STATE.ACTIVE_KK_DATA.no_kk){
      Swal.fire({icon:"warning",title:"Mohon Tunggu",text:"Data Keluarga sedang diproses...",timer:1500,showConfirmButton:false,toast:true,position:"top-end"});
      return;
    }
    const k=STATE.ACTIVE_KK_DATA,
      i=STATE.PENDUDUK.data.find(m=>String(m.no_kk)===String(k.no_kk)&&(String(m.kk_level)==="3"||String(m.kk_level).toUpperCase()==="ISTRI"));
    openFormPenduduk("tambah",null,{
      mode_lahir:true,no_kk:k.no_kk,alamat:k.alamat,dusun:k.dusun,rt:k.rt,rw:k.rw,
      nama_ayah:k.sex=="1"||k.sex=="LAKI-LAKI"?k.nama:"",
      ayah_nik:k.sex=="1"||k.sex=="LAKI-LAKI"?k.nik:"",
      nama_ibu:i?i.nama:"",ibu_nik:i?i.nik:"",
      status_kawin:"1",kk_level:"4",status_dasar:"1"
    });
  }
  else if(tipe==="masuk"){
    if(!STATE.ACTIVE_KK_DATA||!STATE.ACTIVE_KK_DATA.no_kk){
      Swal.fire({icon:"warning",title:"Mohon Tunggu",text:"Data sedang dimuat...",toast:true,position:"top-end",timer:1500,showConfirmButton:false});
      return;
    }
    const k=STATE.ACTIVE_KK_DATA;
    openFormPenduduk("tambah",null,{
      mode_masuk:true,no_kk:k.no_kk,alamat:k.alamat,dusun:k.dusun,rt:k.rt,rw:k.rw,
      status_dasar:"1",status_kawin:"1"
    });
  }
  else if(tipe==="existing"){
    if(!STATE.ACTIVE_KK_DATA||!STATE.ACTIVE_KK_DATA.no_kk){
      Swal.fire({icon:"warning",title:"Mohon Tunggu",text:"Data KK sedang dimuat...",toast:true,position:"top-end",timer:1500,showConfirmButton:false});
      return;
    }
    const modal=document.getElementById("modalTarikAnggota"),
      input=document.getElementById("cariPendudukTarikInput"),
      list=document.getElementById("hasilTarikList");
    if(modal){
      modal.classList.remove("hidden");
      input&&(input.value="",input.focus());
      list&&(list.innerHTML="",list.classList.add("hidden"));
    }else console.error("Modal #modalTarikAnggota tidak ditemukan di HTML");
  }
  else Swal.fire("Info","Fitur ini sedang dikembangkan","info");
};
...
window.editAnggotaKeluarga=function(nik){
  ...
  openFormPenduduk("ubah",nik);
  setTimeout(()=>{
    document.getElementById("formTitlePenduduk").innerHTML=
      '<i class="fas fa-user-edit text-orange-500"></i> Ubah Anggota Keluarga';
  },100);
};

   window.tambahKKBaru=function(){
  const pc=document.getElementById("pendudukContainer"),
        isPenduduk=pc&&!pc.classList.contains("hidden");
  STATE.ORIGIN_VIEW=isPenduduk?"penduduk":"keluarga";

  const d1=document.getElementById("dropdownTambahKK"),
        d2=document.getElementById("dropdownTambahMenu");
  ...
  setTimeout(()=>{
    const t=document.getElementById("formTitlePenduduk");
    if(t)t.innerHTML='<i class="fas fa-folder-plus text-green-600"></i> Buat Kartu Keluarga Baru';
    const i=document.getElementById("p_no_kk");
    if(i){i.value="";i.focus();}
  },100);
};
window.handleCariTarik = debounce(async (keyword) => {
        const list = document.getElementById('hasilTarikList');
        const loading = document.getElementById('loadingTarik');
        ...
        try {
            // Cari penduduk, tapi jangan tampilkan yang sudah ada di KK ini
            const currentNoKK = STATE.ACTIVE_KK_DATA.no_kk;
            const res = await apiCall({ action: "get_penduduk", keyword: keyword, limit: 10 });
            
            list.innerHTML = "";
            
            if (res.status && res.data.list.length > 0) {
                // Filter: Jangan tampilkan orang yang sudah ada di KK ini
                const filtered = res.data.list.filter(p => String(p.no_kk) !== String(currentNoKK));
                
                if (filtered.length === 0) {
                    list.innerHTML = '<li class="p-3 text-center text-sm text-gray-500 italic">Penduduk tidak ditemukan atau sudah ada di KK ini.</li>';
                } else {
                    filtered.forEach(p => {
                        const li = document.createElement('li');
                        li.className = "px-4 py-3 hover:bg-blue-50 dark:hover:bg-gray-700 cursor-pointer border-b dark:border-gray-700 flex justify-between items-center group";
                        li.innerHTML = `
                            <div>
                                <p class="font-bold text-gray-800 dark:text-gray-200 text-sm">${p.nama}</p>
                                <p class="text-xs text-gray-500 dark:text-gray-400">NIK: ${p.nik} <br> KK Asal: ${p.no_kk || 'Tidak Ada'}</p>
                            </div>
                            <button class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold group-hover:bg-green-600 group-hover:text-white transition">Pilih</button>
                        `;
                        li.onclick = () => prosesTarikAnggota(p);
                        list.appendChild(li);
                    });
                }
                list.classList.remove('hidden');
            } else {
                 list.innerHTML = '<li class="p-3 text-center text-sm text-gray-500 italic">Data tidak ditemukan</li>';
                 list.classList.remove('hidden');
            }
        } catch (e) {
            console.error(e);
...
window.prosesTarikAnggota = async function(penduduk) {
        document.getElementById('modalTarikAnggota').classList.add('hidden');
        
        // Target Data (KK yang sedang dibuka)
        const targetKK = STATE.ACTIVE_KK_DATA;
        
        // --- PERBAIKAN: INJEKSI DATA KE MEMORI ---
        // Kita masukkan data orang yang dipilih ke dalam STATE agar openFormPenduduk bisa menemukannya
        if (!STATE.PENDUDUK.data) STATE.PENDUDUK.data = [];
        
        // Cek apakah data sudah ada di memori?
        const indexCek = STATE.PENDUDUK.data.findIndex(d => d.nik == penduduk.nik);
        if (indexCek === -1) {
            // Jika belum ada, masukkan (push)
            STATE.PENDUDUK.data.push(penduduk);
        } else {
            // Jika sudah ada, update datanya (agar fresh)
            STATE.PENDUDUK.data[indexCek] = penduduk;
        }
        // -----------------------------------------

        // Sekarang aman memanggil fungsi ini karena datanya sudah ada di memori
        await openFormPenduduk('ubah', penduduk.nik);
        
        // --- PROSES OVERWRITE (Ganti KK Lama dengan KK Baru) ---
        setTimeout(() => {
            const titleEl = document.getElementById('formTitlePenduduk');
            if(titleEl) titleEl.innerHTML = `<i class="fas fa-exchange-alt text-purple-600"></i> Pindah ke KK: ${targetKK.no_kk}`;
            
            // 1. Isi No KK Baru & Kunci
            const inputNoKK = document.getElementById('p_no_kk');
            if(inputNoKK) {
                inputNoKK.value = targetKK.no_kk;
                inputNoKK.setAttribute('readonly', true);
                inputNoKK.classList.add('bg-yellow-50', 'border-yellow-500'); 
            }
            
            // 2. Isi No KK Sebelumnya (Isi dengan KK lama dia)
            // Pastikan field ini MUNCUL agar data tersimpan
            const wrapperKKSeb = document.getElementById('p_no_kk_sebelumnya');
            if(wrapperKKSeb) {
                // Tampilkan fieldnya jika tersembunyi
                if(wrapperKKSeb.parentElement.classList.contains('hidden')){
                    wrapperKKSeb.parentElement.classList.remove('hidden');
                }
                wrapperKKSeb.value = penduduk.no_kk || ""; 
            }

            // 3. Reset Hubungan Keluarga
            const inputHub = document.getElementById('p_kk_level');
            if(inputHub) {
                inputHub.value = ""; 
                inputHub.focus();
                window.Toast.fire({ icon: 'info', title: 'Silakan tentukan Hubungan Keluarga baru.' });
            }

            // 4. Samakan Alamat dengan KK Baru
            const setVal = (id, val) => { const el = document.getElementById(id); if(el) el.value = val; };
            setVal('p_alamat', targetKK.alamat);
            
            // Trigger update wilayah manual
            const elDusun = document.getElementById('p_dusun');
            if(elDusun) { 
                elDusun.value = targetKK.dusun; 
                elDusun.dispatchEvent(new Event('change'));
                setTimeout(() => {
                    const elRW = document.getElementById('p_rw');
                    if(elRW) { 
                        elRW.value = targetKK.rw; 
                        elRW.dispatchEvent(new Event('change'));
                         setTimeout(() => {
                             const elRT = document.getElementById('p_rt');
                             if(elRT) elRT.value = targetKK.rt;
                         }, 200);
                    }
                }, 200);
            }
...
window.openFormPenduduk = async function(mode, nik = null, defaults = null) {
        ...
        const setSmartVal = (id, val) => {
            const el = document.getElementById(id);
            if (!el) return;
            let v = val != null ? String(val).replace(/'/g,"").trim() : "";
            if(el.type === "date" && (v === "-" || v === "0000-00-00" || v.length < 10)) v = "";
            ...
            if(el.tagName === "SELECT"){
                ...
                if(v !== "" && (el.value === "" || el.value !== v)){
                    ...
                    for(let i=0; i<el.options.length; i++){
                        ...
                        if(o.text.toUpperCase().trim() === u || o.value.toUpperCase().trim() === u){
                            el.selectedIndex = i; f = true; break;
                        }
                    }
                    if(!f && typeof STATE !== "undefined" && STATE.KAMUS){
                        const k = id.replace("p_","");
                        if(STATE.KAMUS[k]){
                            const fk = Object.keys(STATE.KAMUS[k]).find(x => STATE.KAMUS[k][x].toUpperCase() === u);
                            if(fk) el.value = fk;
                        }
                    }
                }
        ...
        const triggerWilayahUpdate = (d, r, t) => {
            const u = document.getElementById("p_dusun");
            const w = document.getElementById("p_rw");
            const x = document.getElementById("p_rt");
            if(u && d) {
                u.value = d; u.onchange();
                if(w && r) {
                    setTimeout(() => {
                        w.value = r; w.onchange();
                        if(x && t) { setTimeout(() => { x.value = t; }, 50); }
                    }, 50);
                }
            }
        };
        ...
        if (mode === "tambah") {
            ...
            if (defaults) {
                ...
                // Isi Ibu jika ada (perbaikan dari chat sebelumnya)
                if(defaults.nama_ibu) setSmartVal("p_nama_ibu", defaults.nama_ibu);
                if(defaults.ibu_nik) setSmartVal("p_ibu_nik", defaults.ibu_nik);
                
                triggerWilayahUpdate(defaults.dusun, defaults.rw, defaults.rt);

                // --- BAGIAN YANG ANDA TANYAKAN ---
                if (defaults.mode_lahir) {
                    // Cek apakah No KK sudah ada (dari Page Keluarga)?
                    const kkSudahAda = (defaults.no_kk && String(defaults.no_kk).length > 0);
                    
                    // 1. Input No KK: Sembunyi jika dari Keluarga, Tampil jika dari Penduduk
                    toggleField("p_no_kk", !kkSudahAda);
                    
                    // 2. Input KK Sebelumnya (Search): Sembunyi jika dari Keluarga, Tampil jika dari Penduduk
                    toggleField("p_no_kk_sebelumnya", !kkSudahAda);
                    
                    if(prefixWilayah) setSmartVal("p_nik", prefixWilayah);
                    
                    // Jika dari Keluarga, isi otomatis No KK-nya
                    if(kkSudahAda) setSmartVal("p_no_kk", defaults.no_kk);

                    setSmartVal("p_kk_level", "4"); // Anak
                    ...
                    const kkSudahAda = (defaults.no_kk && String(defaults.no_kk).length > 0);
                    toggleField("p_no_kk", true);
                    toggleField("p_no_kk_sebelumnya", false); // Masuk biasanya pakai SKPWNI
                    
                    if (kkSudahAda) {
                        setSmartVal("p_no_kk", defaults.no_kk);
                        lockInput(inputNoKK);
                    } else if (prefixWilayah) {
                        setSmartVal("p_no_kk", prefixWilayah);
                    }
                    if (prefixWilayah) setSmartVal("p_nik", prefixWilayah);
                ...
                Object.keys(defaults).forEach(k => {
                    if (!["mode_lahir", "mode_masuk", "alamat", "nama_ayah", "nik_ayah", "nama_ibu", "ibu_nik", "dusun", "rw", "rt", "no_kk"].includes(k)) {
            ...
            if(!data && STATE.KELUARGA?.data) data = STATE.KELUARGA.data.find(d => d.nik == nik);
            if(!data && STATE.ACTIVE_KK_DATA?.nik == nik) data = STATE.ACTIVE_KK_DATA;
            ...
            if (data) {
                ...
                if (imgPreview) {
                    ...
                    imgPreview.src = photoUrl ? (window.convertDriveUrl ? convertDriveUrl(photoUrl) : photoUrl) :
                        `https://ui-avatars.com/api/?name=${encodeURIComponent(data.nama || "User")}&background=random&color=fff&size=200&bold=true`;
                }
...
window.editKepalaKeluarga=function(nik){
    STATE.ORIGIN_VIEW="keluarga";
    document.getElementById("keluargaContainer").classList.add("hidden");
    openFormPenduduk("ubah",nik);
    setTimeout(()=>{document.getElementById("formTitlePenduduk").innerHTML='<i class="fas fa-user-tie text-orange-500"></i> Ubah Kepala Keluarga'},100)
};
...
window.closeFormPenduduk=function(){
  const f=document.getElementById("pendudukFormView"),
        pt=document.getElementById("pendudukTableView"),
        pd=document.getElementById("pendudukDetailView"),
        kd=document.getElementById("keluargaDetailView"),
        kt=document.getElementById("keluargaContainer");

  if(f)f.classList.add("hidden");

  if(STATE.ORIGIN_VIEW==="keluarga_detail"){
    if(kd)kd.classList.remove("hidden");
  }
  else if(STATE.ORIGIN_VIEW==="keluarga"){
...
// --- GANTI FUNGSI INI DI SCRIPT ANDA ---

function generateKopHTML(config, desa) {
    // 1. HELPER DECODE (PENTING: Ini yang memperbaiki masalah kode muncul sebagai teks)
    ...
    // ============================================================
    // OPSI A: HEADER CUSTOM (MASALAH UTAMA ADA DI SINI)
    // ============================================================
    if (config.header_surat && config.header_surat.length > 20) {
        // [FIX] DECODE DULU KODENYA AGAR TIDAK JADI TEXT
        let kop = decodeHtml(config.header_surat); 
        ...
        // Force Style Logo agar tidak pecah
...
async function initEditorPengaturan(selectorId) {
    ...
    // --- HELPER BARU: PEMULIH KODE HTML ---
    // Mengubah &lt;table&gt; menjadi <table> agar terbaca sebagai gambar/tabel
    ...
    tinymce.init({
        ...
        setup: function(editor) {
            editor.on("init", function() {
                ...
                const rawVal = document.getElementById(selectorId).value;
                
                // --- PERBAIKAN DI SINI ---
                // Decode dulu sebelum dimasukkan ke Editor
                const cleanVal = decodeHtmlEntity(rawVal);
                // -------------------------

                if (!cleanVal && selectorId === "setting_header_surat") {
                    // Default jika kosong
                    ...
                    // Masukkan data yang sudah dibersihkan (decoded)
...
window.initViewProfile = async function() {
    const sections = ['#dashboardSection', '#viewSection', '#editSection', '#mapSection', '#pendudukContainer', '#keluargaContainer', '#suratContainer', '#cetakSuratContainer', '#pemerintahContainer', '#userSettingsView'];
    ...
    // [UBAH] Ambil dari user_profile
    const profileStr = localStorage.getItem("user_profile");
    let hasCache = false;
    
    if (profileStr) {
        try {
            renderProfileUI(JSON.parse(profileStr));
            hasCache = true;
        } catch(e){}
    }
    ...
    try {
        ...
        if (res.status) {
            ...
            localStorage.setItem("user_profile", JSON.stringify(res.data)); // Simpan yang baru
...
window.initPengaturanProfile = async function() {
    ...
    if(profileStr) {
        try {
            populateFormSettings(JSON.parse(profileStr));
        } catch(e){}
    }
...
window.initLapor = function() {
    ...
    const sections = ["dashboardSection", "pendudukContainer", "viewSection", "mapSection", "keluargaContainer", "suratContainer", "pemerintahContainer", "userSettingsView", "laporanAdminContainer"];
    ...
    // [UBAH] Cek User Profile
    const profileStr = localStorage.getItem("user_profile");
    if (profileStr) {
        try {
            const user = JSON.parse(profileStr);
            window.STATE_LAPOR_NIK = user.nik || "";
